...
class Classifier_logreg(nn.Module):
    '''
    Implementation of a Sentiment Classifier as specified by the paper

    '''
    def __init__(self, embedding, batch_size):
        super(Classifier_logreg, self).__init__()
        # Infer vocab, embedding_dimension size by the given embedding
        self.vocab_size, self.emb_size = embedding.size()
        # Initialize and copy weights from given embedding
        self.embedding = nn.Embedding(self.vocab_size, self.emb_size)
    ...
    def forward(self, x):
        # Embed review into encoding
        em = self.embedding(x)
        em = torch.sum(em,axis=0)
        zeros = torch.count_nonzero(x-1, dim=0) + torch.count_nonzero(x, dim=0) - x.size()[0]
        em = em.T / zeros

        yHat = self.sigmoid(self.linear(em.T)).squeeze()

        # print(torch.mean(yHat), torch.mean(self.W), self.b)
        return yHat.squeeze()
